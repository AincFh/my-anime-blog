...
export async function searchAnime(keyword: string, limit: number = 10): Promise<BangumiSearchResult[]> {
    ...
    try {
        ...
        if (!response.ok) {
            getLogger().error('Bangumi search HTTP error', { status: response.status });
            return [];
        }
        ...
        getLogger().error('Bangumi search failed', { error: error instanceof Error ? error.message : String(error) });
        return [];
    }
}
...
export async function getAnimeDetail(subjectId: number): Promise<BangumiSubject | null> {
    try {
        ...
        if (!response.ok) {
            getLogger().error('Bangumi detail fetch HTTP error', { status: response.status });
            return null;
        }
        ...
        getLogger().error('Bangumi detail fetch failed', { error: error instanceof Error ? error.message : String(error) });
        return null;
    }
}
...
export async function getAnimeCharacters(subjectId: number): Promise<BangumiCharacter[]> {
    try {
        ...
        if (!response.ok) {
            getLogger().error('Bangumi characters fetch HTTP error', { status: response.status });
            return [];
        }
        ...
        getLogger().error('Bangumi characters fetch failed', { error: error instanceof Error ? error.message : String(error) });
        return [];
    }
}
...
export async function getAnimeStaff(subjectId: number): Promise<BangumiPerson[]> {
    try {
        ...
        if (!response.ok) {
            getLogger().error('Bangumi staff fetch HTTP error', { status: response.status });
            return [];
        }
        ...
        getLogger().error('Bangumi staff fetch failed', { error: error instanceof Error ? error.message : String(error) });
        return [];
    }
}
 ...
 * 从 infobox 提取制作公司等信息
 */
export function extractInfoFromInfobox(infobox: Array<{ key: string; value: string | Record<string, unknown> }> | undefined): {
    studio?: string;
    director?: string;
    originalWork?: string;
} {

...
import { getLogger } from '~/utils/logger';export interface EmailOptions {
  to: string;
  subject: string;
  html: string;
  text?: string;
...
export async function sendEmailViaMailChannels(
  ...
  try {
    ...
    getLogger().error('Send email via MailChannels failed', { error: error instanceof Error ? error.message : String(error) });
    return false;
  }
}
...
export async function sendEmailViaResend(
  ...
  try {
    ...
    getLogger().error('Send email via Resend failed', { error: error instanceof Error ? error.message : String(error) });
    return false;
  }
}

...
async function fetchSiteContentFromNotion(
    ...
    if (kv) {
        try {
            ...
            if (cached) {
                getLogger().debug('SiteContent cache hit');
                return JSON.parse(cached);
            }
        } catch { /* ignore */ }
    }

    getLogger().debug('Fetching site content from Notion');
...
export async function getSiteContent(
    ...
    try {
        ...
        getLogger().error('Fetch site content failed', { error: errMsg });
        return { success: false, data: [], error: "获取站点内容失败" };
    }
}
